...
export interface TgMessage {
  ...
  /** Sent as "File"; a compressed photo arrives in `photo` instead. */
...
/** `image` picks `sendPhoto` (inline) over `sendDocument` (a download card). */
...
/** Telegram's way to ask for one typed answer. */
...
export interface TelegramClient {
  ...
  /** `null` clears; Telegram allows one bot reaction per message. */
  ...
  /** `text` shows as a toast. */
  answerCallbackQuery(id: string, text?: string): Promise<void>;
  /** `name` is the basename Telegram stored it under. */
