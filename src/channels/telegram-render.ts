...
// How a reply looks on Telegram: markdown → Bot API HTML. Telegram's parser
// rejects the whole message on any tag outside its documented set (b, i, s,
// code, pre, a), so text is escaped first and only those are reintroduced.
...
/** A cut inside a <pre> block is fine: Telegram closes the tag on its own. */
export const chunk = (html: string): string[] => chunkText(html, MAX_CHARS);

// --- next-step buttons -------------------------------------------------------

/** `callback_data` caps at 64 bytes (~21 CJK characters), so buttons send an
 *  index and the label is read back off the message's own keyboard. */
export const OFFER_PREFIX = "sg:";
// Buttons in one row split its width evenly, so packing is budgeted by display
// width, not count.
const ROW_WIDTH = 26;
const ROW_BUTTONS = 3;

/** CJK, fullwidth punctuation and emoji take two cells. */
const displayWidth = (label: string): number =>
  [...label].reduce((n, ch) => n + ((ch.codePointAt(0) ?? 0) > 0x2e7f ? 2 : 1), 0);

/** A row that is already wide stops accepting: a long label squeezed in
 *  truncates everything beside it. */
