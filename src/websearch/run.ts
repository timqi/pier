...
const LANGUAGE_MODES: readonly LanguageMode[] = ["auto", "preserve", "expand"];
const BACKENDS: readonly Backend[] = ["anthropic", "openai"];
const FETCH_MODES = Object.keys(FETCH_LIMITS) as FetchMode[];
const MAX_DOMAINS = 20;

const oneOf = <T extends string>(value: unknown, allowed: readonly T[], name: string): T | undefined => {
  if (value === undefined) return undefined;
  if (typeof value === "string" && (allowed as readonly string[]).includes(value)) return value as T;
  throw new Error(`${name} must be one of ${allowed.join(", ")}`);
};
const domainList = (value: unknown, name: string): string[] | undefined => {
  if (value === undefined) return undefined;
...
export function parseWebParams(raw: unknown): WebParams {
  if (typeof raw !== "object" || raw === null) throw new Error("params must be an object");
  const p = raw as Record<string, unknown>;
  if (p.op === "search") {
    if (typeof p.query !== "string" || p.query.trim().length < 2) throw new Error("query must be at least 2 characters");
  ...
  if (p.op === "fetch") {
    ...
    if (p.prompt !== undefined && typeof p.prompt !== "string") throw new Error("prompt must be a string");
...
export const runWeb = (params: WebParams, ctx: WebContext, note: Note, signal?: AbortSignal): Promise<WebResult> =>
  params.op === "search" ? webSearch(params, ctx, note, signal) : webFetch(params, ctx, note, signal);

export async function webSearch(params: SearchParams, ctx: WebContext, note: Note, signal?: AbortSignal): Promise<WebResult> {
...
export async function webFetch(params: FetchParams, ctx: WebContext, note: Note, signal?: AbortSignal): Promise<WebResult> {
