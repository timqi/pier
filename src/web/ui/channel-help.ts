...
export const slackThreadHelp = (): HTMLElement =>
  helpBadge("How Slack threads work here", [
    ...
    "Commands are bare words after a mention: `@bot settings`, `@bot stop`, `@bot bind <code>`. `@bot s <question>`, as the first message of a thread, drafts a session — directory, model, reasoning — and Start runs the question in it. Slack's client swallows an unregistered `/command` before it ever reaches an app.",
  ]);
...
export const larkThreadHelp = (): HTMLElement =>
  helpBadge("How Lark topics work here", [
    ...
    "Commands are slash words: `/stop`, `/settings`, `/bind <code>`. `/s <question>` or `s <question>`, outside any topic, drafts a session — directory, model, reasoning — and Start runs the question in it. A bare @mention with nothing else also opens the settings panel.",
  ]);
