...
describe("openDb", () => {
  ...
  it("refuses a database written by a newer Pier instead of serving it", () => {
    ...
    expect(() => openDb(path)).toThrow(/at schema 99, this Pier speaks 28/);
  });
  ...
  it("indexes the global run list, on a database that predates it", () => {
    ...
    before.exec(UNDO_28 + UNDO_27 + UNDO_26 + UNDO_23 + UNDO_22 + UNDO_20 + UNDO_17 + " PRAGMA user_version = 16");
  ...
  it("indexes the two columns the scheduler sweeps, on a database that predates them", () => {
    ...
    before.exec(
      "DROP INDEX task_runs_callback_state; DROP INDEX task_messages_state;" +
        ...
        UNDO_28 + UNDO_27 + UNDO_26 + UNDO_23 + UNDO_22 + UNDO_20 + UNDO_17 + " PRAGMA user_version = 14",
    );
    before.close();
  ...
  it("carries every task's next run into a column an older database never had", () => {
    ...
    before.exec(
      "DROP INDEX tasks_due; ALTER TABLE tasks DROP COLUMN next_run_at;" +
        UNDO_28 + UNDO_27 + UNDO_26 + UNDO_23 + UNDO_22 + UNDO_20 + UNDO_17 + " PRAGMA user_version = 15",
    );
  ...
  it("unpins every session a database pinned while pinned meant listed", () => {
    ...
    const before = openDb(path);
    before.exec(
      UNDO_28 + UNDO_27 + UNDO_26 + UNDO_23 + UNDO_22 + UNDO_20 +
  ...
  it("clears the marks a database made before unread meant the workbench's own", () => {
    ...
    before.exec(
      "INSERT INTO session_state(session_id, unread, sort) VALUES ('im', 1, NULL), ('own', 1, 0);" +
        UNDO_28 + UNDO_27 + UNDO_26 + UNDO_23 + UNDO_22 + " PRAGMA user_version = 20",
    );
    before.close();
  ...
  it("seeds the working set from the pinned rows, and drops the rest of the pins", () => {
    const path = dbPath();
    const before = openDb(path);
    before.exec(UNDO_28 + UNDO_27 + UNDO_26 + UNDO_23 + UNDO_22 + UNDO_20);
  ...
  it("moves channel credentials into the vault verbatim, and a name filed first stays", () => {
    const path = dbPath();
    const before = openDb(path);
    before.exec(UNDO_28 + UNDO_27 + UNDO_26 + " PRAGMA user_version = 23");
  ...
  it("adds the launch column to the conversations a database already routes", () => {
    const path = dbPath();
    const before = openDb(path);
    before.exec(UNDO_28 + UNDO_27 + UNDO_26 + " PRAGMA user_version = 25");
  ...
  it("drops the decision, progress and reply messages a database kept for the mid-run channel", () => {
    const path = dbPath();
    const before = openDb(path);
    before.exec(UNDO_28 + UNDO_27 + UNDO_26 + " PRAGMA user_version = 24");
