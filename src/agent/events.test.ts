...
describe("toSessionEvents", () => {
  const cases: { name: string; input: PiEvent; expected: unknown[] }[] = [
    ...
    {
      // The probe docs/design/06-desk.md demanded, and its answer: the
      // transcript renders nothing for a compaction (see toChatTurns below),
      // so this event is the only thing that ever says the context shrank.
...
describe("toChatTurns", () => {
  ...
  // The probe from docs/design/06-desk.md, kept as a test because it is the
  // reason the `context-compacted` event exists. Pi replaces the summarized
  // entries with one `compactionSummary` message (session-manager.ts,
  // sessionEntryToContextMessages) — a role this rebuild does not know, so the
  // compaction leaves *no* trace in the transcript a reload renders. If that
  // ever changes, this expectation changes with it and the event's reason for
  // existing has to be re-argued.
