...
| `POST /api/packages/update` | body `{source?}` → `update(source)`; `source` absent updates every unpinned npm/git package. Answers `{packages}`, the rows it moved, when done. 404 unknown, 409 for a local/path or version-pinned source (nothing to move) or while another operation runs. Never called by anything but a Console click |
