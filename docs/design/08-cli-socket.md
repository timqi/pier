...
| `/task` | `{sessionId, params}` — `params` exactly the object the `task` tool takes: `operation` is `run`, `message`, `save`, `list`, `cancel` or `recover` ([09-tasks-cli.md](09-tasks-cli.md)) | `200 {result}`, the tool's return value; `422 {error}` with the tool's own message for anything it threw — `handleTaskTool` is the one validator, and the CLI does none |
