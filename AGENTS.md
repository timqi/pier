...
| `channels/` | ~4.16k | 4.3k | four adapters in the same five-file shape (adapter, api, render, outbound, panel); the shared layer holds only what would otherwise be copied — fence balancer, event dedup, `attach.ts` |
| `web/` | ~10.9k | 8.6k — **second look due** | largest and least tested. Password boundary, Settings console, Files view, Terminal (pty + WS + emulator), Web Push (RFC 8291/8292, dependency-free), extension and managed-tool switches — each a surface, none a dependency. But the number in this column had drifted ~2.3k behind the code, which is the one failure this table cannot absorb: a proxy nobody re-measures is not measuring. The features above have their sentences; the last reading does not, and the next change here owes a real second look before it adds anything |
| `tasks/` | ~2.59k | 2.5k | one outbox engine for delivery proof/backoff/ceiling, durable control-message state, required failure paths, and the owner seam that protects Pier-created definitions |
| root `src/*.ts` | ~2.43k | 1.3k | one-reason modules each: secure credentials, service/update ops, restart ledger, and the managed CLI tools — ubix bootstrap, strict parse of its JSON, provision/removal order, one cross-process sync lock in `pier.db` |
