...
| root `src/*.ts` | ~1.45k | 1.3k — second look done: the instance layer now owns secure credentials, service/update operations and the graceful-restart ledger; each remains a one-reason module, and the growth since is one schema migration, the composition line that hands the push surface its dependencies, and the enabled-extension set — stored by shape only, because the catalog is code and naming it here would drag the Pi SDK into the instance layer, and +44 for the librarian seam, which is composition by definition: the librarian's marker, schedule and prompt are the bus's and creating a task is the task service's, and the two areas do not import each other |
