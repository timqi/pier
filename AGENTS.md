...
| `channels/` | 4.2k | two adapters in one shape (adapter, api, render, panel, outbound); Slack also `slack-cli` (`pier slack`) and `slack-transcript`; the shared layer holds only what would otherwise be copied, plus the handoff — the one cross-surface binding and the one root post, whose failure table is principle 5's |
